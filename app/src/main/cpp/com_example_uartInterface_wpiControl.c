/* DO NOT EDIT THIS FILE - it is machine generated */
#include <jni.h>
#include <wiringPi.h>
#include <softPwm.h>
#include <dlfcn.h>
#include <wiringPiSPI.c>
#include <wiringPiI2C.h>
#include <wiringSerial.h>
#include <errno.h>
#include <android/log.h>
/* Header for class com_example_wiringop_wpiControl */

#define LOGI(...)  __android_log_print(ANDROID_LOG_INFO,LOG_TAG,__VA_ARGS__)
#define LOGE(...)  __android_log_print(ANDROID_LOG_ERROR,LOG_TAG,__VA_ARGS__)
#define LOG_TAG "wpiControl"

#ifndef _Included_com_example_uartinterface_controls_WpiControl
#define _Included_com_example_uartinterface_controls_WpiControl
#ifdef __cplusplus
extern "C" {
#endif
/*
 * Class:     com_example_uartinterface_controls_WpiControl
 * Method:    wiringPiSetup
 * Signature: ()I
*/
JNIEXPORT jint JNICALL Java_com_example_uartinterface_controls_WpiControl_wiringPiSetup
        (JNIEnv *env, jclass obj) {
    LOGI("Entering wiringPiSetup");
    return wiringPiSetup();
}
/*
 * Class:     com_example_uartinterface_controls_WpiControl
 * Method:    pinMode
 * Signature: (II)V
*/
JNIEXPORT void JNICALL Java_com_example_uartinterface_controls_WpiControl_pinMode
        (JNIEnv *env, jclass obj, jint pin, jint mode)
{
    return pinMode(pin, mode);
}

/*
 * Class:     com_example_uartinterface_controls_WpiControl
 * Method:    pullUpDnControl
 * Signature: (II)V
*/
JNIEXPORT void JNICALL Java_com_example_uartinterface_controls_WpiControl_pullUpDnControl
        (JNIEnv *env, jclass obj, jint pin, jint pud)
{
    return pullUpDnControl(pin, pud);
}

/*
 * Class:     com_example_uartinterface_controls_WpiControl
 * Method:    digitalRead
 * Signature: (I)I
*/
JNIEXPORT jint JNICALL Java_com_example_uartinterface_controls_WpiControl_digitalRead
        (JNIEnv *env, jclass obj, jint pin)
{
    return digitalRead(pin);
}

/*
 * Class:     com_example_uartinterface_controls_WpiControl
 * Method:    digitalWrite
 * Signature: (II)V
*/
JNIEXPORT void JNICALL Java_com_example_uartinterface_controls_WpiControl_digitalWrite
        (JNIEnv *env, jclass obj, jint pin, jint value)
{
    digitalWrite(pin, value);
}

/*
 * Class:     com_example_uartinterface_controls_WpiControl
 * Method:    wiringPiSPIGetFd
 * Signature: (I)I
*/
JNIEXPORT jint JNICALL Java_com_example_uartinterface_controls_WpiControl_wiringPiSPIGetFd
        (JNIEnv *env, jclass obj, jint channel)
{
    return wiringPiSPIGetFd(channel);
}

/*
 * Class:     com_example_uartinterface_controls_WpiControl
 * Method:    wiringPiSPIDataRW
 * Signature: (I[BI)I
*/
JNIEXPORT jint JNICALL Java_com_example_uartinterface_controls_WpiControl_wiringPiSPIDataRW
        (JNIEnv *env, jclass obj, jint channel, jbyteArray data, jint len) {
    unsigned char *cdata = NULL;
    int ret = -1;
    LOGI("Entering wiringPiSPIDataRW");
    cdata = (unsigned char *) (*env)->GetByteArrayElements(env, data, NULL);
    ret = wiringPiSPIDataRW(channel, cdata, len);
    (*env)->ReleaseByteArrayElements(env, data, (jbyte *) cdata, 1);
    return ret;
}

/*
 * Class:     com_example_uartinterface_controls_WpiControl
 * Method:    wiringPiSPISetupMode
 * Signature: (IIII)I
*/
JNIEXPORT jint JNICALL Java_com_example_uartinterface_controls_WpiControl_wiringPiSPISetupMode
        (JNIEnv *env, jclass obj, jint channel, jint port, jint speed, jint mode) {
    return wiringPiSPISetupMode(channel, port, speed, mode);
}

/*
 * Class:     com_example_uartinterface_controls_WpiControl
 * Method:    wiringPiSPISetup
 * Signature: (II)I
*/
JNIEXPORT jint JNICALL Java_com_example_uartinterface_controls_WpiControl_wiringPiSPISetup
        (JNIEnv *env, jclass obj, jint channel, jint speed) {
    return wiringPiSPISetup(channel, speed);
}

/*
 * Class:     com_example_uartinterface_controls_WpiControl
 * Method:    serialOpen
 * Signature: (Ljava/lang/String;I)I
 */
JNIEXPORT jint JNICALL Java_com_example_uartinterface_controls_WpiControl_serialOpen
        (JNIEnv *env, jclass obj, jstring device, jint baud) {
    const char *nativeString = (*env)->GetStringUTFChars(env, device, 0);
    int ret = serialOpen(nativeString, baud);
    if (ret < 0) LOGE("Unable to open serial device: %s", strerror(errno));
    (*env)->ReleaseStringUTFChars(env, device, nativeString);
    return ret;
}

/*
 * Class:     com_example_uartinterface_controls_WpiControl
 * Method:    serialClose
 * Signature: (I)V
 */
JNIEXPORT void JNICALL Java_com_example_uartinterface_controls_WpiControl_serialClose
        ( JNIEnv *env,  jclass obj, jint fd)
{
    serialClose(fd);
}

/*
 * Class:     com_example_uartinterface_controls_WpiControl
 * Method:    serialFlush
 * Signature: (I)V
 */
JNIEXPORT void JNICALL Java_com_example_uartinterface_controls_WpiControl_serialFlush
        ( JNIEnv *env,  jclass obj, jint fd)
{
    serialFlush(fd);
}

/*
 * Class:     com_example_uartinterface_controls_WpiControl
 * Method:    serialPutchar
 * Signature: (IC)V
*/
JNIEXPORT void JNICALL Java_com_example_uartinterface_controls_WpiControl_serialPutchar
        (JNIEnv *env, jclass obj, jint fd, jbyte c)
{
    serialPutchar(fd, c);
}

/*
 * Class:     com_example_uartinterface_controls_WpiControl
 * Method:    serialPuts
 * Signature: (ILjava/lang/String;)V
 */
JNIEXPORT void JNICALL Java_com_example_uartinterface_controls_WpiControl_serialPuts
        (JNIEnv *env,  jclass obj, jint fd, jstring s)
{
    const char *nativeString = (*env)->GetStringUTFChars(env, s, 0);
    serialPuts(fd, nativeString);
    (*env)->ReleaseStringUTFChars(env, s, nativeString);
}

/*
 * Class:     com_example_uartinterface_controls_WpiControl
 * Method:    serialDataAvail
 * Signature: (I)I
 */
JNIEXPORT jint JNICALL Java_com_example_uartinterface_controls_WpiControl_serialDataAvail
        ( JNIEnv *env,  jclass obj, jint fd)
{
    return serialDataAvail(fd);
}

/*
 * Class:     com_example_uartinterface_controls_WpiControl
 * Method:    serialGetchar
 * Signature: (I)I
 */
JNIEXPORT jint JNICALL Java_com_example_uartinterface_controls_WpiControl_serialGetchar
        ( JNIEnv *env,  jclass obj, jint fd)
{
    return serialGetchar(fd);
}

/*
 * Class:     com_example_uartinterface_controls_WpiControl
 * Method:    wiringPiI2CRead
 * Signature: (I)I
*/
JNIEXPORT jint JNICALL Java_com_example_uartinterface_controls_WpiControl_wiringPiI2CRead
        (JNIEnv *env, jclass obj, jint fd)
{
    return wiringPiI2CRead(fd);
}

/*
 * Class:     com_example_uartinterface_controls_WpiControl
 * Method:    wiringPiI2CReadReg8
 * Signature: (II)I
*/
JNIEXPORT jint JNICALL Java_com_example_uartinterface_controls_WpiControl_wiringPiI2CReadReg8
        (JNIEnv *env, jclass obj, jint fd, jint reg)
{
    return wiringPiI2CReadReg8(fd, reg);
}

/*
 * Class:     com_example_uartinterface_controls_WpiControl
 * Method:    wiringPiI2CReadReg16
 * Signature: (II)I
*/
JNIEXPORT jint JNICALL Java_com_example_uartinterface_controls_WpiControl_wiringPiI2CReadReg16
        (JNIEnv *env, jclass obj, jint fd, jint reg)
{
    return wiringPiI2CReadReg16(fd, reg);
}

/*
 * Class:     com_example_uartinterface_controls_WpiControl
 * Method:    wiringPiI2CWrite
 * Signature: (II)I
*/
JNIEXPORT jint JNICALL Java_com_example_uartinterface_controls_WpiControl_wiringPiI2CWrite
        (JNIEnv *env, jclass obj, jint fd, jint data)
{
    return wiringPiI2CWrite(fd, data);
}

/*
 * Class:     com_example_uartinterface_controls_WpiControl
 * Method:    wiringPiI2CWriteReg8
 * Signature: (III)I
*/
JNIEXPORT jint JNICALL Java_com_example_uartinterface_controls_WpiControl_wiringPiI2CWriteReg8
        (JNIEnv *env, jclass obj, jint fd, jint reg, jint data)
{
    return wiringPiI2CWriteReg8(fd, reg, data);
}

/*
 * Class:     com_example_uartinterface_controls_WpiControl
 * Method:    wiringPiI2CWriteReg16
 * Signature: (III)I
*/
JNIEXPORT jint JNICALL Java_com_example_uartinterface_controls_WpiControl_wiringPiI2CWriteReg16
        (JNIEnv *env, jclass obj, jint fd, jint reg, jint data)
{
    return wiringPiI2CWriteReg16(fd, reg, data);
}

/*
 * Class:     com_example_uartinterface_controls_WpiControl
 * Method:    wiringPiI2CSetup
 * Signature: (Ljava/lang/String;I)I
*/
JNIEXPORT jint JNICALL Java_com_example_uartinterface_controls_WpiControl_wiringPiI2CSetup
        (JNIEnv *env, jclass obj, jint devId)
{
    return wiringPiI2CSetup(devId);
}

/*
 * Class:     com_example_uartinterface_controls_WpiControl
 * Method:    wiringPiI2CSetupInterface
 * Signature: (Ljava/lang/String;I)I
*/
JNIEXPORT jint JNICALL Java_com_example_uartinterface_controls_WpiControl_wiringPiI2CSetupInterface
        (JNIEnv *env, jclass obj, jstring device, jint devId)
{
    const char *str;
    int ret;
    str = (*env)->GetStringUTFChars(env, device, NULL);
    ret = wiringPiI2CSetupInterface(str, devId);
    (*env)->ReleaseStringUTFChars(env, device, str);
    return ret;
}

/*
 * Class:     com_example_uartinterface_controls_WpiControl
 * Method:    getGpioPhysToWpi
 * Signature: ()[I
*/
JNIEXPORT  jint JNICALL Java_com_example_uartinterface_controls_WpiControl_getGpioInfo
        (JNIEnv *env, jclass obj, jintArray javaArrWpi, jobjectArray javaArrPhysName )
{
    g_info gpio_info;
    jint *physToWpi;
    jstring physNames;

    int ret = get_gpio_info(&gpio_info);

    if(-1 == ret)
        return -1;
    physToWpi = (*env)->GetIntArrayElements(env, javaArrWpi, NULL);
    int len = (*env)->GetArrayLength(env, javaArrWpi);
    for(int i=0; i<len; i++){
        physToWpi[i] = gpio_info.physToWpi[i];
    }
    (*env)->ReleaseIntArrayElements(env, javaArrWpi, physToWpi, 0);

    jsize stringArrayLength = (*env)->GetArrayLength(env, javaArrPhysName);
    for(int i=0; i<stringArrayLength; i++){
        physNames = (*env)->NewStringUTF(env, gpio_info.physNames[i]);
        (*env)->SetObjectArrayElement(env, javaArrPhysName,i,physNames);
        (*env)->DeleteLocalRef(env, physNames);
    }
    return gpio_info.pin_max;
}

#ifdef __cplusplus
}
#endif
#endif
